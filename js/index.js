...
async function searchByCoordinates(latitude, longitude) {
  let weatherApi = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=cf41e0a19f634f81a4f65456241612&q=${latitude},${longitude}&days=3`
  );
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      ...
      function (error) {
        ...
        searchByCoordinates(30.0444, 31.2357); // Cairo
      }
    );
  } else {
...
async function searchCity(cityName) {
  let weatherApi = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=cf41e0a19f634f81a4f65456241612&q=${cityName}&days=3`
  );
  let apiResponse = await weatherApi.json();
